...
public class GestureActivity extends BaseActivity implements View.OnTouchListener {
    ...
    public class simpleGestureListener extends GestureDetector.SimpleOnGestureListener {
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            if(sumX<0){
                if(Math.abs(sumX)>200)	{  shortToast("You scroll from left to right");}
            }
            if(sumX>0){
            if(Math.abs(sumX)>200)	{  shortToast("You scroll from right to left");}
        }
            if(sumY<0){
        ...
        @Override
        public boolean onSingleTapUp(MotionEvent e) { // onLongPress, onScroll,onFling,onShow,onDown
...
//        @Override
//        public boolean onScroll(MotionEvent e1,MotionEvent e2, float distanceX, float distanceY ) { // onLongPress, onScroll,onFling,onShow,onDown
//            UtilLog.d("Gesture","onScroll");
//            UtilLog.d("Gesture","distanceX: ", +distanceX);
//            UtilLog.d("Gesture","distanceY: ", +distanceY);
//            sumX+=distanceX;
